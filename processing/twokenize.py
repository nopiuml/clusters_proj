...
# tha mporoysa na prosthesw gia tis lekseis poy emfanizontai San Francisco, Los Angeles, National Aeronautics and Space Administration (NASA)
...
# datetime   # m/d or mm/dd or   d/m or dd/mm  /yy or /yyyy
datetime_long = r"^(?:(1[0-2]|0?[1-9])/(3[01]|[12][0-9]|0?[1-9])"+ \
r"(1[0-2]|0?[1-9])/(3[01]|[12][0-9]|0?[1-9])"+ \
r"|"+ \
r" (3[01]|[12][0-9]|0?[1-9])/(1[0-2]|0?[1-9]))" + \
r"/(?:[0-9]{2})?[0-9]{2}$"

datetime_short = r"^(?:(1[0-2]|0?[1-9])/(3[01]|[12][0-9]|0?[1-9])"+ \
r"(1[0-2]|0?[1-9])/(3[01]|[12][0-9]|0?[1-9])"+ \
r"|"+ \
r" (3[01]|[12][0-9]|0?[1-9])/(1[0-2]|0?[1-9]))"

datetime = r"datetime_long|datetime_short"


# TO BE FIXED
"""
 - there are some mentions with spaces ex: @ laternative
 - some shit like that [?][?][?][?]'
 - dates fucking dates 05/13/15
 - //
"""
...
def simpleTokenize(text):
    ...
    splitGoods = []
    # start = 0; stop = len(indices); step = 2)
    for i in range(0, len(indices), 2):
        goodstr = splitPunctText[indices[i]:indices[i+1]]
        splitstr = goodstr.strip().split(" ")
        # metatrepw ola ta goodstr se mikra
        splitstr = [x.lower() for x in splitstr]
    ...
    for tok in zippedStr:
        # lower case in order to be normalized
        # se ena emoticon bgazei :D -> :d
       splitStr.extend(splitToken(tok.lower()))
    zippedStr = splitStr
